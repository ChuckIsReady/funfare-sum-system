...
detail:function (req, res) {
    ...
    Funfare.findOne(req.params.id).exec( function(err, model) {
        if (model != null)
            return res.view('detail', {'funfare': model});
        else
            return res.view('message', {'message': "No such place"});
    }); 
},
// json function
...
admin: function(req, res) {
    Funfare.find().exec( function(err, funfares) {
        return res.view('admin', {'funfares': funfares});
    });
},
create: function(req, res) {
        if (req.method == "POST") {
            ...
            Funfare.create(req.body.Funfare).exec( function(err, model) {
                ...
                return res.view('message', {'message': 'Create Successfully!'});
            });
        } else {
...
update: function(req, res) {
    if (req.method == "GET") {
        Funfare.findOne(req.params.id).exec( function(err, model) {
            if (model == null)
        return res.view('message', {'message': "No such funfare!"});
            else
                return res.view('update', {'funfare': model});
        });
    } else {
        Funfare.findOne(req.params.id).exec( function(err, model) { new Date().setTime()
                ...
                model.travelDatesEnd = new Date(req.body.travelDatesEnd);
                model.save();
    return res.view('message', {'message': "Record updated"});
        }); 
    }
},
                 ...
                 if(req.query.region=="All regions"){
                         ...
                         .exec( function (err, funfares) {
                                         ...
                                         .exec( function(err, value) {
                                             ...
                                             return res.view('search', {'funfares': funfares, 'count':pages,'query':req.query});
                                         });
                         });
          ...
          funfares = "";
            ...
            .exec( function(err, funfares) {
                ...
                .exec( function(err, value) {
                    ...
                    return res.view('search', {'funfares': funfares, 'count':pages,'query':query});
                });
            });         
    }
