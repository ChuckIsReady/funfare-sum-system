...
//Rare exceptions to indicate a bug in the _funfares code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_funfares = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_funfares]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.funfares.module"
